...
# Write the selected profile to a temporary file for AWS_PROFILE environment variable
echo "export AWS_PROFILE=$profile_name" > ./tmp_setenv.sh

echo "AWS SSO profile '$profile_name' is successfully configured. The AWS_PROFILE environment variable has been set in a temporary environment file. You might need to run 'source ./tmp_setenv.sh' to apply changes in a new terminal."

# Source the temporary environment file in the current session
source ./tmp_setenv.sh
